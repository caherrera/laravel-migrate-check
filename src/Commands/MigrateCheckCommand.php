...
class MigrateCheckCommand extends BaseCommand {
	 ...
	 */
	protected $signature
		= 'migrate:check {--database= : The database connection to use.}
	...
	public function handle() {
		...
		if ( $this->output->isVerbose() ) {
			$this->output->write('Pending migrations :');
		}
		$this->output->write( count( $pendingMigrations ) );
		if ( $this->output->isVerbose() ) {
			$this->output->writeln('');
		}
